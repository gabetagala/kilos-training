...
// The coach's clips live in public/voice/<slug>.m4a (Brian, generated by
// scripts/generate-voice.mjs; a personal recording under the same name takes
// over — see VOICE-RECORDING.md). Every clip is fetched and decoded ONCE into
// an AudioBuffer: buffer sources start sample-accurate and, once the context
// is unlocked by the first tap, play from timers on iOS — which fresh
// HTMLAudio and speechSynthesis do not. fetch needs no gesture either, so a
// clip's existence is no longer probed through an <audio> element's
// canplaythrough (which iOS only fires reliably inside a tap).
const rhClipCache = new Map(); // slug → url | null (null = known missing)
const rhClipBuffers = new Map(); // slug → AudioBuffer (beat-accurate playback)
const rhClipLoads = new Map(); // slug → in-flight Promise<AudioBuffer|null>
function rhLoadClip(slug) {
  if (rhClipBuffers.has(slug)) return Promise.resolve(rhClipBuffers.get(slug));
  if (rhClipLoads.has(slug)) return rhClipLoads.get(slug);
  const load = (async () => {
    for (const ext of ['m4a', 'mp3', 'webm']) {
      const url = `/voice/${slug}.${ext}`;
      try {
        const res = await fetch(url);
        if (!res.ok) continue;
        // the dev server answers a missing file with index.html — decoding
        // is the real existence test, not the status
        const buf = await ensureCtx().decodeAudioData(await res.arrayBuffer());
        rhClipCache.set(slug, url);
  ...
  rhClipLoads.set(slug, load);
  load.then(() => rhClipLoads.delete(slug));
  return load;
}
...
let rhTtsToken = 0;
// Our own bound on an utterance's life. WebKit can leave `speaking` stuck
// true after cancel(); trusted alone, that held the mic for the whole
// session — every count and coach line dropped.
let rhTtsUntil = 0;
function rhTtsActive() {
  if (Date.now() >= rhTtsUntil) return false;
  try {
...
function rhPlayBuf(slug, opts = {}) {
  ...
  if (!buf) return false;
  const ctx = ensureCtx();
  // a count that can't sound NOW must not queue up on a parked context and
  // fire late as a ghost
  ...
  const mic = mayInterject(
    {
      ...
      announceLive: !!rhAnnSrcs || rhTtsActive(),
      bufUntil: rhBufUntil,
    },
    { cut: opts.cut },
  );
...
// The whole phrase is scheduled on the context clock in one go — each clip
// starts the sample the previous one ends, so nothing rides on onended
// latency — and it is scheduled ONLY once the context is actually running.
// A parked context (screen lock, a call, Siri) used to accept the first clip
// and play it whenever it woke, seconds late, while a deadline check dropped
// the rest: "next —" and no name. Now the phrase waits up to 1.5s for the
// context and is dropped WHOLE past that — a cue belongs to its moment.
let rhAnnSrcs = null; // the scheduled sources of the phrase on the mic
// Chain generation: stopping a source FIRES its onended, so every takeover
// bumps the token and a stale phrase's callbacks find themselves orphaned
// (without it, cutting off "next — band lateral raise" resurrected its
// remaining clips on top of "twelve reps — go": his 2026-08-14 report).
let rhAnnGen = 0;
let rhClipAudio = null;
function rhStopAnnounce() {
  rhAnnGen += 1; // orphan any pending chain BEFORE stop() triggers onended
  for (const src of rhAnnSrcs || []) {
    try {
      src.stop();
    } catch {}
  }
  rhAnnSrcs = null;
  // the HTMLAudio fallback is the same voice on the same mic — a buffer
  // chain or TTS taking over used to leave it talking underneath
  try {
    rhClipAudio?.pause();
  } catch {}
  rhClipAudio = null;
}
// Run fn(true) once the context is running — now, or within limitMs of a
// resume; past that, fn(false) so the caller drops its cue instead of
// playing it late.
function rhWhenRunning(ctx, fn, limitMs = 1500) {
  ...
  const onChange = () => {
    if (ctx.state === 'running') settle(true);
  };
  ctx.addEventListener?.('statechange', onChange);
  const timer = setTimeout(() => settle(false), limitMs);
  try {
    ctx.resume().then(onChange, () => {});
  } catch {}
}
// delayMs: the tone lead — the voice starts as the step's tone decays, not
// under it (simultaneous tone + first word was the "mixed" audio at every
// set start and set end).
function rhPlayClipSeq(slugs, { delayMs = 0 } = {}) {
  const ctx = ensureCtx();
  rhStopBuf(); // announcements outrank tempo words
  rhStopAnnounce();
  rhStopTts(); // …and replace a still-talking TTS announcement
  const gen = rhAnnGen; // claimed after the stops — this phrase owns the mic
  const bufs = slugs.map((sl) => rhClipBuffers.get(sl)).filter(Boolean);
  if (!bufs.length) return;
  const totalMs = bufs.reduce((sum, b) => sum + b.duration, 0) * 1000;
  // claim the mic from the tone lead on — a count must not sneak in
  rhAnnounceUntil = Date.now() + delayMs + totalMs + 400;
  rhWhenRunning(ctx, (ok) => {
    if (gen !== rhAnnGen) return; // a newer voice took the mic while we waited
    if (!ok) {
      rhAnnounceUntil = 0; // too late — dropped whole, not half
      return;
    }
    ...
    rhAnnSrcs = srcs;
    // re-anchor on the real start — the wait may have eaten the estimate
    rhAnnounceUntil = Date.now() + delayMs + totalMs + 400;
...
// HTMLAudio fallback — only while a clip is still decoding (see rhCueSay).
function rhPlayClips(urls, { delayMs = 0 } = {}) {
  rhStopBuf();
  rhStopAnnounce();
  rhStopTts();
  ...
  rhAnnounceUntil = Date.now() + delayMs + urls.length * 1100;
  const done = () => {
  ...
  if (delayMs > 0) setTimeout(() => next(0), delayMs);
  else next(0);
}
...
let rhCueGen = 0; // a newer cue orphans one still waiting on its decode
function rhCueSay(parts, ttsText, opts = {}) {
  if (!rhVoiceOn) return;
  const gen = ++rhCueGen;
  const play = () => {
    if (parts.length && parts.every((slug) => rhClipBuffers.get(slug))) {
      rhPlayClipSeq(parts, opts);
      return;
    }
    const urls = parts.map((slug) => rhClipCache.get(slug));
    if (urls.length && urls.every(Boolean)) {
      rhPlayClips(urls, opts);
      return;
    }
    rhSay(ttsText ?? parts.join(' ').replace(/-/g, ' '), opts);
  };
  // A clip still decoding (the first cue right after open, a slow phone) is
  // a beat away from being the right voice — wait for it rather than fall
  // through to TTS, which iOS won't voice. Capped so a cue can never arrive
  // late; a newer cue orphans the wait.
  const pending = parts.filter(
    (slug) => !rhClipBuffers.get(slug) && rhClipLoads.has(slug),
  );
  if (!pending.length) {
    play();
    return;
  ...
  rhStopAnnounce();
  rhStopTts();
  rhAnnounceUntil = Date.now() + 700; // hold the mic while we wait
  Promise.race([
    Promise.all(pending.map(rhLoadClip)),
    new Promise((resolve) => setTimeout(resolve, 600)),
  ]).then(() => {
    if (gen === rhCueGen) play();
  });
}

function rhSay(text, { delayMs = 0 } = {}) {
  ...
  try {
    ...
    const est = ttsWindowMs(text);
    rhAnnounceUntil = Date.now() + delayMs + est;
    rhTtsUntil = Date.now() + delayMs + est * 2;
    const speak = () => {
      if (token !== rhTtsToken) return;
...
// The voice starts as the step's tone decays — long enough that the tone is
// out of the way, short enough that the word still belongs to the beat.
const TONE_LEAD_MS = { work: 350, rest: 250, switch: 300, finish: 550 };

// Arriving at a step: distinct tone + spoken cue, so ears alone carry you.
// WHAT is said is pure (workout/announce.js) and runs under
// scripts/verify-voice.mjs over every reachable queue, so a cue cannot reach
// the player without a clip behind it.
function rhAnnounceStep(step) {
  ...
  if (step.kind === 'work') {
    ...
    rhCue('switch');
    lead = TONE_LEAD_MS.switch;
  } else if (step.phase === 'REST') {
    rhCue('rest');
    lead = TONE_LEAD_MS.rest;
  } // BREATHE micro-rests stay silent — the 3-2-1 ticks carry them
  navigator.vibrate?.(step.kind === 'work' ? 120 : 60);

  const cue = announceCue(rhQueue, rhIdx, {
    minimal: rhCoachMinimal(),
    exercises: GUIDED_EXERCISES,
    nextWorkLabel,
  });
  if (cue) rhCueSay(cue.parts, cue.text, { delayMs: lead });
}
...
function rhStopGuide(silent = false) {
  ...
  if (was && !silent) {
    rhCue('rest');
    rhCueSay(['rest'], 'Rest', { delayMs: TONE_LEAD_MS.rest });
  }
...
function rhComplete(overflowMs = 0) {
  ...
  // A landing after a background gap is PAUSED (below). Announcing it here
  // said "go" to a phone just out of a pocket, and the play tap then had
  // nothing left to say — the "too early" cue. Leave it unannounced; play
  // announces a fresh step. A self-paced landing still says where you are.
  if (!backgrounded || next.manual) rhAnnounceStep(next);
...
function openRehabPlayer(session, saved = null, variantOverride = null) {
  ...
  // …and load the voice clips once, so cues never wait on a fetch: what THIS
  // queue will say at either coach level, plus the tempo words, milestones
  // and the session's own form lines. Decoded up front: iOS Safari only
  // allows fresh HTMLAudio inside a tap, so timer-driven announcements must
  // play through the (once-unlocked) AudioContext — same path as the beat
  // words.
  const slugs = new Set([
    'go',
    'hold',
  ...
  rhQueue.forEach((_, i) => {
    for (const minimal of [true, false]) {
      const cue = announceCue(rhQueue, i, {
        minimal,
        exercises: GUIDED_EXERCISES,
        nextWorkLabel,
      });
      for (const p of cue?.parts || []) slugs.add(p);
    }
  });
  for (const exId of new Set(rhQueue.map((s) => s.exId))) {
    for (const c of FORM_CUES[exId] || []) slugs.add(c.slug);
  }
  slugs.delete(TTS_ONLY);
  for (const slug of slugs) rhLoadClip(slug);
}
...
function rhFinish() {
  ...
  if (finBlock?.name && finAttempted) {
    ...
    if (finBlock.mode === 'fortime') {
      ...
      setTimeout(
        () =>
          openFinisherScorePrompt(promptCfg, last?.score, (score) => {
            ...
            const rec = h.find(
              (x) =>
                x.date === capturedEntry.date && x.name === capturedEntry.name,
            );
            if (!rec) return;
            rec.finisher = {
              name: finBlock.name,
