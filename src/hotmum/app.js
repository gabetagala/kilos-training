...
// Bumped when the shape of a live run changes in a way that makes an
// in-flight session un-resumable. The 2026-08-22 rewrite renamed the sessions
// (lower-a → lower, lower-b → full), dropped the dose/stage fields and rebuilt
// every queue, so a state saved before it would have resumed at an index
// pointing into a completely different session.
const RUN_SCHEMA = 2;
...
// The PLAYABLE session — rotations interleaved (program.js). The sheet uses
// the un-interleaved one so it reads like her plan; everything that indexes
// into the queue has to use this one, because `st.bi` points into it.
//
// MEMOISED, because findBlock() calls this and findBlock is on the hot path:
// the preload alone asks for it once per work step, then every render asks
// twice more. Un-memoised that was roughly a thousand full session rebuilds —
// progress(), kneeEasy() and expandRotations(), ~18 fresh objects each — per
// session, on the loop CLAUDE.md says must never stall.
let runCache = { key: null, session: null };
function runSession() {
  const key = `${run.sessionId}|${run.easyKnee}|${run.day}`;
  if (runCache.key !== key) {
    runCache = {
      key,
      session: playable(
...
// The sheet's big title. It used to split a trailing " A"/" B" onto its own
// emphasised line for "Lower A" / "Lower B"; those session names are gone and
// every caller now passes "Lower + Knee", "Walk", "Yoga" — nothing can match.
const dayTitle = (name) => esc(name.toUpperCase());
...
function renderProgram() {
  ...
  const sessions = HOTMUM_SESSIONS.map((s) => {
    const p = playable(prepped(s.id, { day }));
  ...
  app.innerHTML = `
    <div class="screen has-nav scr-program">
      ...
      <p class="fine pane-wide">Every set is a stretch of work with a rep
        target in it. Alice names the movement and the reps, then leaves you
        to it — three tones tell you when the set is ending.</p>
    </div>
    ${nav('program')}`;
    ...
    .map((h, i) => {
      return `<div class="row">
        <div>
          ...
          <div class="row-s">${esc(h.date)} · ${esc(historyDetail(h))}</div>
        </div>
  ...
  app.innerHTML = `
    <div class="screen has-nav scr-me">
      ...
      <button class="row row-tap" data-edit="voice">
        <div><div class="row-t">Coach voice</div><div class="row-s">Alice names each movement and its reps</div></div>
...
// What a logged day was called. One place, because there are three renderers
// (home recents, the athlete log, the finish card) and they had drifted: two
// printed "Walk" for a yoga session, and a record written under an older
// session id printed "Walk" for a lifting day.
function historyName(h) {
  if (h.kind === 'session') return getSession(h.sessionId)?.name || 'Session';
  return getMovement(h.moveId).name;
}
function historyDetail(h) {
  if (h.kind === 'session') return `${h.sets} sets · ${mmss(h.secs)}`;
  return `${Math.round((h.secs || 0) / 60)} min`;
}
...
function openPicker() {
  const options = [
    ...HOTMUM_SESSIONS.map((s) => ({
      ...
      sub: s.blurb,
      meta: `${estimateSessionMins(playable(prepped(s.id)))} MIN`,
...
function openSession(id) {
  ...
  function paint() {
    ...
    const mins = estimateSessionMins(playable(s));
    ...
    const rows = sessionParts(s)
      .map((part) => {
        // Her plan says "Alternate:" / "Rotate through:" on these — say so,
        // because a rotation feels completely different from straight sets.
        const rounds = Math.max(...part.blocks.map((b) => b.sets || 1));
        const note = s.rotate?.includes(part.key)
          ? `<span class="part-note">rotate through · ${rounds} rounds</span>`
          : '';
        return `<div class="part-h"><span class="lbl lbl-sm">${esc(part.label)}</span>${note}</div>${part.blocks.map(rowFor).join('')}`;
      })
      .join('');
...
function startSession(sessionId, easyKnee = false) {
  const day = dayNumber();
  const session = playable(prepped(sessionId, { easyKnee, day }));
  run = {
    kind: 'session',
    sessionId,
    easyKnee,
...
function renderPlayer() {
  ...
  // `resting` is set HERE now. It used to be written by tick() every frame,
  // and went out with the tempo bloom — leaving .player.resting unreachable in
  // style.css, so a rest looked exactly like a work set. Nothing rewrites
  // className any more, so once per step is enough.
  // `data-stage` stays an attribute: the desktop grid collapses to one column
  // when there's no pose to show (a walk, a yoga class).
  app.innerHTML = `
    <div class="screen player${st.kind === 'rest' ? ' resting' : ''}" id="pl" data-stage="${teach ? 'on' : 'off'}">
      <div class="bloom"></div>
      <div class="pl-top">
...
function restore() {
  ...
  if (s.kind === 'walk') {
    ...
    const session = playable(
      prepped(s.sessionId, { easyKnee: s.easyKnee, day: s.day }),
    );
    if (!session) {
      drop(K.active);
      return false;
